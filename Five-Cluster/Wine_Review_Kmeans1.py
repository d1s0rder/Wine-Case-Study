...
fig.savefig('silhouette_kmeans1.png')

kmeans_5 = KMeans(n_clusters = 5)
kmeans_5.fit(wine_no_text)
...
for i, cc in enumerate(kmeans_5.cluster_centers_):
    ...
    ax1.set_xticklabels(wine_no_text.columns[0: 4], rotation = 30)
    ax1.set_xticks(np.arange(4))
    ax1.legend()
    
    ax2.bar(np.arange(4) + i*0.2, cc[4: 8], width = 0.15, label = 'Cluster{}'.format(i))
    ax2.set_xticklabels(wine_no_text.columns[4: 8], rotation = 30)
    ax2.set_xticks(np.arange(4))
    ax2.legend()
    
    ax3.bar(np.arange(4) + i*0.2, cc[8: 12], width = 0.15, label = 'Cluster{}'.format(i))
    ax3.set_xticklabels(wine_no_text.columns[8: 12], rotation = 30)
    ax3.set_xticks(np.arange(4))
    ax3.legend()
    
    ax4.bar(np.arange(4) + i*0.2, cc[12: 16], width = 0.15, label = 'Cluster{}'.format(i))
    ax4.set_xticklabels(wine_no_text.columns[12: 16], rotation = 30)
    ax4.set_xticks(np.arange(4))
    ax4.legend()
fig.savefig('kmeans1_visualization.png')
...
normal_result.to_csv('kmeans1_five_cluster.csv')
