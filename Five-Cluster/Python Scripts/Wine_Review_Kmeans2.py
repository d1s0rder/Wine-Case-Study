...
for cluster in clusters:
    ...
    kmeans_text.fit(wine_no_text_combine)
    score = silhouette_score(wine_no_text_combine, kmeans_text.labels_)
...
fig.savefig('Silhouette_Kmeans2.png')

kmeans_5 = KMeans(n_clusters = 5)
kmeans_5.fit(wine_no_text_combine)
...
for i, cc in enumerate(kmeans_5.cluster_centers_):
    ax1.bar(np.arange(4) + i*0.2, cc[0: 4], width = 0.15, label = 'Cluster{}'.format(i))
    ax1.set_xticklabels(wine_no_text_combine.columns[0: 4], rotation = 30)
    ax1.set_xticks(np.arange(4))
    ax1.legend()
    
    ax2.bar(np.arange(4) + i*0.2, cc[4: 8], width = 0.15, label = 'Cluster{}'.format(i))
    ax2.set_xticklabels(wine_no_text_combine.columns[4: 8], rotation = 30)
    ax2.set_xticks(np.arange(4))
    ax2.legend()
    
    ax3.bar(np.arange(4) + i*0.2, cc[8: 12], width = 0.15, label = 'Cluster{}'.format(i))
    ax3.set_xticklabels(wine_no_text_combine.columns[8: 12], rotation = 30)
    ax3.set_xticks(np.arange(4))
    ax3.legend()
   
    ax4.bar(np.arange(4) + i*0.2, cc[12: 16], width = 0.15, label = 'Cluster{}'.format(i))
    ax4.set_xticklabels(wine_no_text_combine.columns[12: 16], rotation = 30)
    ax4.set_xticks(np.arange(4))
    ax4.legend()
    
    ax5.bar(np.arange(1) + i*0.2, cc[16], width = 0.15, label = 'Cluster{}'.format(i))
    ax5.set_xlabel('year')
    ax5.set_xticks([])
    ax5.legend() 
fig.savefig('kmeans2_visualization.png')

label = pd.Series(kmeans_5.labels_, name = 'cluster_label')
label_no_text_combine = wine_no_text_combine.join(label)
...
def logreg_top_four(int):
    label_no_text_combine['convert_label'] = np.where(label_no_text_combine['cluster_label'] == int, 1, 0) # Convert the label to 1 and other label to 0
    X_train,X_test,Y_train,Y_test=train_test_split(label_no_text_combine.drop(['cluster_label', 'convert_label'], axis = 1),
                                                   label_no_text_combine['convert_label'],test_size=0.25)
...
normal_result.to_csv('kmeans2_five_cluster.csv')

label_no_text_combine = wine_no_text_combine.join(label)
def kmeans_top_bottom_ten(cluster_label):
    text_df = label_no_text_combine[label_no_text_combine['cluster_label'] == cluster_label]
    label_df = kmeans_5.cluster_centers_[cluster_label].reshape(1,-1)
...
for i in range(5):
    ...
    top_center.to_csv('kmeans2_top_center_{}.csv'.format(i))
    bottom_center.to_csv('kmeans2_bottom_center_{}.csv'.format(i))

    top_rating =top_center.nlargest(20, 'points')
    top_rating.to_csv('kmeans2_top_rating_{}.csv'.format(i))
    ...
    price_bin.to_csv('kmeans2_price_bin_{}.csv'.format(i))
